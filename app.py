...
def ask_question_route():
    ...
    if query_text:      
        ...
        if (ans_dict['doc']==selected_file) and (list(ans_dict.keys())[1]== query_text):
            res_ans = ans_dict[query_text][selected_language]
        else:
        # Determine if the answer needs to be updated
        # elif (len(ans_dict) == 0) or ((list(ans_dict.keys())[1] != query_text) and (ans_dict['doc'] == selected_file)) or \
        #    ((list(ans_dict.keys())[1] == query_text) and ans_dict['doc'] != selected_file):
